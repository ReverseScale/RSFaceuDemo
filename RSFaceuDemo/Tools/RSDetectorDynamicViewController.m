//
//  RSDetectorDynamicViewController.m
//  RSFaceuDemo
//
...
@interface RSDetectorDynamicViewController ()<AVCaptureVideoDataOutputSampleBufferDelegate>
...
@implementation RSDetectorDynamicViewController
...
-(CIImage *)inputCIImageForDetector:(CIImage *)image{
    ...
    if (!(faceArray.count==1)) {
        ...
        if ([self.detectorDelegate respondsToSelector:@selector(detectorDynamicLocateFace:)]) {
            [self.detectorDelegate detectorDynamicLocateFace:CGRectZero];
        }
    ...
    for (CIFeature *f in faceArray){
        ...
        if ([self.detectorDelegate respondsToSelector:@selector(detectorDynamicLocateFace:)]) {
            [self.detectorDelegate detectorDynamicLocateFace:faceViewBounds];
        }
        ...
        if(faceFeature.hasRightEyePosition){
            ...
            if ([self.detectorDelegate respondsToSelector:@selector(detectorDynamicLocateRightEyeForFace:)]) {
                [self.detectorDelegate detectorDynamicLocateRightEyeForFace:rightEyeBounds];
            }
            
        }else{
            if ([self.detectorDelegate respondsToSelector:@selector(detectorDynamicLocateRightEyeForFace:)]) {
                [self.detectorDelegate detectorDynamicLocateRightEyeForFace:CGRectZero];
            }
        }
        ...
        if(faceFeature.hasLeftEyePosition){
            ...
            if ([self.detectorDelegate respondsToSelector:@selector(detectorDynamicLocateLeftEyeForFace:)]) {
                [self.detectorDelegate detectorDynamicLocateLeftEyeForFace:leftEyeBounds];
            }
            
        }else{
            if ([self.detectorDelegate respondsToSelector:@selector(detectorDynamicLocateLeftEyeForFace:)]) {
                [self.detectorDelegate detectorDynamicLocateLeftEyeForFace:CGRectZero];
            }
        }
